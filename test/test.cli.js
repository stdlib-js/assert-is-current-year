...
tape( 'the command-line interface tests if an input argument corresponds to the current year', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'1992\'; require( \''+fpath+'\' );"'
	];
...
tape( 'the command-line interface supports use as a standard stream', opts, function test( t ) {
	...
	function done( error, stdout, stderr ) {
		if ( error ) {
			...
			expected = [ false, true, false ].join( '\n' )+'\n';
